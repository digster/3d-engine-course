...
- **Built by hand, because building them is the point:** the math library (`vec2/3/4`, `mat3/4`, quaternions — no GLM), the software rasterizer, an OBJ parser, the ECS, the renderer and frame architecture, the scene and asset systems, arena/pool allocators, and the physics engine (collision, contact generation, and a constraint solver).
...
The arc below is the **required shape** of the course. You may refine lesson boundaries, add lessons, or reorder within a module — you may **not** shrink scope. Target **100–115 lessons** total (roughly 450–550 student-hours). Estimated lesson counts per module are minimums in spirit.

  - *Amended from the original band ("75–95 lessons, roughly 300–500 student-hours") and the original nine-module shape, at the user's explicit direction, after two external reviews of the published outline.* The course sat at **exactly 95 lessons / 444 h** — the ceiling — with three rendering fundamentals missing and physics allocated 25 h, about the same as the quaternion arc alone. Both reviewers independently flagged the same three gaps (**mipmaps, transparency, antialiasing**) from the outline alone; mipmaps in particular had been promised to the student by name six times, including in a doc comment shipped in `engine/include/engine/gfx/texture.hpp`. Physics was **expanded rather than limited**, so it outgrew Module 7 and became **Module 8**, pushing Professional Polish & Capstone to **Module 9**. New shape: **10 modules, 107 lessons, ~510 h**.
  - What was knowingly given up: **177 `Module 8` references across 65 files had to be rewritten**, 39 of them published lesson pages — the largest edit ever made to already-shipped prose. It was affordable only because the mapping was total (every `Module 8` in the corpus meant the polish module) and verifiable afterwards. The bare `8.N` form was **not** swept: of 58 candidates, only 4 were lesson references and the rest were intra-page section headings, exercise numbers and measurements. §9's "approved lessons are never renumbered" is not violated — no published lesson's own number changed, and 6.9 was deliberately left in place so Lesson 6.8's six references to it and both nav labels still resolve.
  - The bookkeeping this exposed is now checked rather than trusted: **`docs/_template/check-curriculum.py`** verifies that module subtotals, headline totals, badges, nav chains and internal links all agree with the lesson table. Run it whenever a lesson lands or a number moves. It found three dead prerequisite links in 6.8 and a stale `next` in 5.1 on its first run.
...
**Module 5 — Becoming an Engine (≈ 10–12 lessons).** **The Refactor** (see §8): engine as a static library + demo executables, public API design, physical structure (`engine/include/engine/...`). The platform/application layer. Logging, assertions, and an explicit error-handling strategy. Resource **handles** (generational indices) vs raw pointers — why engines do this. Asset system v1. **An ECS built from scratch across multiple lessons:** why OOP scene trees creak at scale, data-oriented design, cache lines and why they dominate, then a justified choice of archetype vs sparse-set storage and a full implementation. Transform hierarchy on top of the ECS. Camera system and input mapping (actions, not keycodes). Dear ImGui integration for debug UI. A debug-draw system (lines, boxes, spheres) the rest of the course will lean on. **A checkpoint game built against the public API** — the first thing in the course written from *outside* the boundary 5.1 declared, and therefore the first real test of it.

**Module 6 — Advanced Rendering (≈ 16–18 lessons).** The linear-vs-sRGB lesson done rigorously — the gamma lesson every graphics programmer needs once and forever. Radiometry-lite → what a BRDF is → Lambert re-understood as a BRDF → microfacet theory → **Cook–Torrance PBR derived intuitively**, with the NDF, geometry, and Fresnel terms each visualized separately. A material system. glTF 2.0 loading. Normal mapping with the full TBN derivation. Shadow mapping (bias and acne explained with diagrams, PCF, then cascades). **Texture minification: mipmap generation and level selection, trilinear filtering, and anisotropy** — promised to the student by name in 3.9 and 4.7, and required by IBL, since a prefiltered environment map is a mip chain. **Transparency: alpha masking vs blending, premultiplied alpha, depth-write rules, and draw order.** HDR pipeline, tonemapping, bloom, and a post-processing stack architecture. **Antialiasing: geometric vs shading aliasing, with one technique implemented.** Skybox and image-based lighting (conceptual-to-practical depth: irradiance and prefiltered environment maps). Frustum culling with AABBs (optional BVH extension), feeding the instancing already built in 4.5–4.6. A lightweight frame-graph-style organization of the renderer. Text and 2D overlay rendering.

**Module 7 — Motion: Rotation, Animation, Audio (≈ 8–10 lessons).** Rotations, the deep dive: Euler angles and their pathologies → axis-angle → **quaternions derived by first fully understanding complex-number rotation in 2D** → slerp. Skeletal animation: skinning math, sampling, and blending. SDL3 audio: audio streams and mixing, a small sound-engine layer, and 3D spatialization (attenuation and panning).

**Module 8 — Physics (≈ 12–14 lessons).** Physics from first principles, and given room to be *correct* rather than merely plausible. Integrators (explicit vs semi-implicit Euler, shown numerically why one explodes), forces and gravity, and linear rigid bodies. **Angular dynamics: torque, the inertia tensor derived via the parallel-axis theorem, and the basis change that carries it to world space** — with the `mat3` operators an outer product needs, added at the point of need. Collision detection: sphere/AABB/OBB and the Separating Axis Theorem with real diagrams, then **GJK for convex distance and EPA for penetration depth**, because the SAT's axis list stops at boxes and papers assume GJK. A uniform-grid broadphase. **Contact manifolds and their persistence across frames**, keyed on feature ids. Impulse-based response with restitution and Coulomb friction, then **a sequential-impulse solver with warm starting, islands, and sleeping** — which is what makes a stack settle instead of sink, and redeems the promise made back in 1.8 to "iterate until the budget is spent". **Constraints and joints (hinge, ball-socket) generalized from the contact solver's Jacobian, and ragdolls** hung off the Module 7 skeleton. A character controller, taught separately and explicitly *not* as a rigid body: grounding, slopes, step-up and tunnelling are design-intent problems, not Newtonian ones. **Forbidden as ever:** Bullet, PhysX, Jolt — building it is the point.

**Module 9 — Professional Polish & Capstone (≈ 10–12 lessons).** Multithreading and a job system (task basics, data hazards, safety rules). CPU and GPU profiling with optimization case studies performed *on our own engine*. Custom allocators (arena, pool) and integrating them where they pay off. Serialization and a scene file format. Hot reloading (shaders first, then assets). **The editor:** ImGui-based scene hierarchy, inspector, and transform gizmos. Packaging and distribution per platform. A pragmatic testing strategy for engine code. A documentation pass: public API docs and an HTML docs site. **Capstone: a complete small 3D game built exclusively against the engine's public API** — the proof that the engine is professional grade.
...
- Each lesson is **one `.html` file with no build tooling of any kind**, styled by the shared course stylesheet at `docs/shared/course.css` and driven by the shared page script at `docs/shared/course.js` — both **linked, not embedded**, so there is exactly one copy of each in the repository. KaTeX or MathJax via CDN is permitted for math typesetting, but any crucial equation must also remain legible from the surrounding prose if the CDN is unreachable.
  - *Amended from the original rule ("fully self-contained, no external assets, CSS embedded identically in every lesson") at the user's explicit direction, once the duplication reached 18% of `docs/`.* The **no build step** guarantee is unchanged and non-negotiable: pages link the shared files straight off the filesystem, so `docs/index.html` still opens by double-clicking, offline, with no server. Verified in Chromium, Firefox and WebKit, including the upward `../shared/` traversal from `docs/lessons/`.
  - What was knowingly given up: a **single lesson file is no longer portable on its own** — emailed or saved in isolation it renders unstyled. Lessons are read as a tree, from `docs/index.html`.
  - A wrong relative href fails **silently** (an unstyled, inert page — not an error). `docs/_template/apply-shared.py` computes and verifies the correct prefix per page; run it whenever a page is added or moved.
...
- **Encouraged:** small inline vanilla-JavaScript interactive widgets when they materially aid intuition — a drag-a-vector dot-product visualizer, a slider-driven frustum/projection demo — written inline in the page that uses them, no frameworks, no build step. Page-specific widgets stay **outside** the `SHARED-SCRIPT` markers, where the stamper cannot reach them.
...
a documented public C++ API; an SDL_GPU-based forward PBR renderer with shadow-mapped lights, HDR, tonemapping, and a post-processing stack; skybox and image-based lighting; an asset pipeline covering images, OBJ, and glTF; a handle-based resource system; a from-scratch ECS runtime with transform hierarchy; skeletal animation; a rigid-body physics engine with angular dynamics, GJK/EPA convex collision, persistent contact manifolds, a warm-started sequential-impulse solver, joints and ragdolls, and a character controller; 3D audio; input mapping; ImGui-based tooling and editor (hierarchy, inspector, gizmos); profiling hooks; serialization and a scene format; hot reload for shaders and assets; a job system; cross-platform CMake builds; sample demos; a complete capstone game built solely on the public API; and an HTML documentation site.
