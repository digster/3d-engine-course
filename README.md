...
**Status:** curriculum and conventions published; lessons in progress — **Modules 0–4 are complete**, **Module 5 is complete** — all 12 lessons, ~62 h, closed out of order by its checkpoint game — and **Module 6 is complete** — all 18 lessons, ~93 h, the longest module in the course — **Module 7 is complete** — all 8 lessons, ~47 h — and **Module 8 is under way** (**93 of 107**).
...
slider. A physics engine computes where things are from how they are **moving**, and before mass or
shapes or contacts there is one question in the way — you have a position, a velocity and an
acceleration; advance them by `h` seconds. The answer that occurs to everybody first is
**unconditionally unstable**: not inaccurate, not needs-a-smaller-step, but wrong at every step size
there is. The whole lesson turns on one number. For a linear restoring force a step is a **2×2
matrix**, and a determinant is the factor a matrix scales area by — explicit Euler's is
`1 + h²ω²`, greater than one always; semi-implicit Euler's is **exactly 1**, because moving one line
above another makes two terms cancel algebraically. On a 1 Hz spring at 60 Hz that surplus is
**1.0966% per step**: ×1.92 energy per second, **×1.13 × 10¹⁷ in a minute**, a one-centimetre
overlap becoming 3,357 km. The fix is measured at **0.994 ns against 0.963 ns** per body per step —
a free lunch, which is the rarest thing in engineering. Along the way: why *order of accuracy is
blind to all of it*, because order asks about `h → 0` at fixed time while a game asks about
`t → ∞` at fixed `h`; what semi-implicit Euler **actually** conserves, which is not energy but a
sheared cousin of it held to **4.6 × 10⁻⁶** over 3,600 float steps while the true energy wobbles by
exactly `hω`; why that shadow quantity being an ellipse rather than a hyperbola *is* the stability
limit `hω < 2`, bisected at **0.3182939 against 0.3183099**; and why **bounded is not the same as
right** — at `hω = 1.95` the orbit is perfectly stable and swings by a factor of forty. Gravity
forgives every integrator (all three err by `±½·a·h·t`, *linearly*, so your first falling-cube demo
is fine) and a contact forgives none, which is the argument — made before meeting the machinery —
for why Lesson 8.10 will solve impulses instead of penalty springs. The narrow phase then arrives in four lessons — [8.4](docs/lessons/08-04-collision-primitives.html)'s Separating Axis Theorem, [8.5](docs/lessons/08-05-gjk.html)'s GJK, [8.6](docs/lessons/08-06-epa.html)'s EPA and [8.7](docs/lessons/08-07-contact-manifolds.html)'s contact manifolds — and 8.7 is where it stops being arithmetic about a pair and becomes something a solver can stand a crate on. 8.6 returns how deep two shapes overlap, which way to push them apart and a pair of witness points, and a crate resting on a floor still will not sit still, because **a contact force acts at a point and therefore exerts no torque about that point**. Measured with the same gravity, the same impulses and the same normal in both arms: four contact points settle at 1.16 mm of penetration and 5.2 °/s of residual rotation; the same contact reduced to its deepest single point sinks **21.27 mm** and rocks at **27.99 °/s**, permanently — and the uncomfortable half is that it still carries roughly the same *moment*, by chattering, which is exactly why single-point contact looks nearly fine in a demo and is unusable in a game. The statics is derivable first (`m·g·(w/2)` = 49.05 N·m, bisected at 47.87) and the geometry needed a new interface: 8.5's claim that a convex set is *determined* by its support function is true, and `support(d)` still returns **one point** when the argmax is a whole square — 4,000 directions from a cone a thousandth of a degree wide return **all four corners**. So the shapes learn to return a whole feature and the two are clipped against each other with Sutherland–Hodgman, the algorithm 3.3 wrote for the near plane. Then persistence, and its trap: over a crate's landing a ten-micrometre position match finds **0.00%** of the contacts while an id match finds 82%, and hold the crate still and *both* find 100% — a position matcher passes every test you would write for it and fails when things move. The lesson also contains two mistakes worth more than the code: a control that convicted the manifold of the *test's* own error, and a knob written for a real hazard, measured at zero flips over 208,000 pairs, and taken back out. Then [8.8](docs/lessons/08-08-broadphase.html) zooms all the way out, because **you cannot afford to run any of that**: 8.7's whole chain re-measures at 432&nbsp;ns a pair, and `n(n−1)/2` of those blows a 60&nbsp;Hz frame at **n = 278** — a small pile of crates. A broadphase asks the cheap question of every pair and is allowed to be wrong in exactly one direction — *it may report pairs that do not touch; it may not omit one that does* — and everything in the lesson falls out of that asymmetry. The grid is hashed and rebuilt every frame; the two hard parts are duplicates (the usual fixes cost **45%** and 5% of the entire broadphase, while the exact owner-cell rule costs three `max` calls — and the closed form for how many duplicates it removes agrees with the grid's own counter at **3271 and 3271**) and the teapot in the stadium (one ground plate wants **80,802 cells** to itself, three times what two thousand crates want). Along the way the famous `floor`-versus-cast bug measures at **zero pairs lost**, because a range-walk grid needs only a *monotone* cell map — while the innocent mistake beside it loses **66.7%**; the cell-size rule of thumb becomes a two-term cost model whose constants turn out to be properties of the machine rather than of the scene; and the crossover comes out honest: **below about a hundred bodies the whole stage is a net loss**. Then something moves: [8.9](docs/lessons/08-09-impulse-response.html) is the first file in `engine::phys` that writes a velocity, and it gives the obvious answer — a spring proportional to the overlap — **164 tuned settings** before rejecting it, whereupon the measurement refuses the section's own claim. A penalty contact is not unstable, it is *tuned*: the best of the 164 lands the crate perfectly at 108 mm of penetration or holds it at 12.6 mm while it bounces 367 forever, and what actually kills it is **transfer**, because the same spring under a 200 kg crate lets it 962 mm into a half-metre floor. An impulse has no stiffness to be stable about, because a collision is an *event*: one linear equation per contact in one scalar unknown, whose constant is the effective mass — rearranged into a form that *proves* the division is safe and checked two ways over 200,000 contacts. Restitution then brings the strangest artifact in the module, a ball that hops **forever** at a height the lesson predicts in closed form to 1.000&times;, because gravity is applied before the solver looks. And [8.10](docs/lessons/08-10-sequential-impulses.html) is the loop around it, plus the two failures the loop cannot fix on its own. Iteration contracts the residual by a measured **0.53 per sweep** — and a four-point manifold on a *single* crate is already a system, because three quarters of the effect of an impulse at a corner is rotation and rotation is what the four points share. Warm starting closes a loop four lessons in the making and takes a settled tower from *collapsing* (2,020 mm of sink) to 40 mm at the same cost — and, in the process, exposes a bug three lessons old: the cached friction impulses are **coordinates in a basis nobody stored**, and the basis flips by ninety degrees on 0.96% of frames. A position correction repairs the overlap no velocity solve can see, and the default is the one that does *not* throw a crate spawned 200 mm inside the floor **99 mm into the air**. Islands partition the level on one rule — a body that cannot move is not a bridge, which is **20 islands against 1** — and sleeping takes a settled yard's solve from 131 µs to **3**, provided the sleep test runs *after* the solve, because before it every resting body in the scene is travelling at `g·h`. It ends where it must: **eight sweeps hold five crates and do not hold ten**, which is what a Gauss–Seidel solver is, and the reason 8.11 writes a constraint as a Jacobian. And [8.11](docs/lessons/08-11-constraints-and-joints.html) does: a constraint is **one row of twelve numbers and two bounds**, a contact normal turns out to be exactly such a row (checked to **1.9 × 10⁻⁷** on two hundred real contacts), and the general form is **27% faster** than the hand-written one because it stores `M⁻¹Jᵀ` — the prediction it was handed said the opposite. A rod is a contact without its clamp, a rope is a contact turned inside out, a ball-socket is three coupled rows solved as one exact 3×3 block, and a hinge adds two angular rows, a limit that gets speculation for free because a hinge always knows its angle, and a motor whose stall torque bisects to exactly `m·g·d`. A velocity-level joint drifts along a tangent by a Pythagorean recurrence the simulation tracks to 1.7 × 10⁻⁷ and loses **ρ(ωh)²** of its energy every step; Baumgarte, caught throwing crates out of floors in 8.10, turns out to *conserve* a turning joint's energy and to leave 0.35 J of spin in a limb pulled back into its socket. And one number arrives three times — the parallel-axis ratio **ρ = m·d²/(I_cm + m·d²)** sets a pendulum's period, a joint's dissipation, and the rate a hinge limit converges against its pin, **0.7481 per sweep** for a door, which is why a door with no restitution still bounces off its stop. [8.12](docs/lessons/08-12-ragdolls.html) puts those joints on a skeleton: 7.7's humanoid becomes **eleven capsules** with Dempster's segment masses, four hinges and six ball-sockets with a **swing cone and a twist range**, and a jogging character is handed to the solver in mid-stride and back. The angles are a swing–twist split, not Euler angles (Euler's sensitivity is **609.9** at an arm raised forward, swing–twist's 1.55); the swing's row is exact, and the twist's is the **half-way axis over cos(φ/2)** — a row about the bone is wrong by the swing plane's turning, which round a loop is the loop's solid angle: **Codman's paradox**, to three decimals. Kinematic bodies are *steered* by velocity, so the handoff keeps the character's momentum exactly (**204.40 kg·m/s**) — the chord is semi-implicit Euler's own velocity — and the return blends from the pose read off the bodies, moving no bone by more than 0.06 mm. The ragdoll also found **two bugs the module had shipped** — 8.11's position pass never repaired a violated two-ended limit, and 8.10's islands never let a moving kinematic body wake anything — and fixed both with the earlier harnesses still agreeing with their pages. [8.13](docs/lessons/08-13-character-controller.html) closes the module with the one body a game must *not* simulate: the player. A rigid capsule on 8.10's solver hangs on a wall mid-jump at any stick speed above **g·h/μ** (27 cm/s at μ = 0.6), slides 2.91 m after the stick is released and climbs a 20 cm ledge only at 8 m/s — Newton being right about everything the design wants false — so the character becomes a massless capsule that asks one question, answered by a **shape cast**. A cast is Newton's method on a convex distance and is safe because each step stops at a separating plane, which is why it must step from GJK's certified *lower* bound (stepped from `distance`, 377 of 6,049 casts ended inside the skin). The rest is policy with a number: clip the *original* motion against every surface or walk back and forth in a corner at 58 cm/s; a round bottom climbs a **90.8 mm** curb for free, stands 219 mm past an edge and needs a step-up to carry it up to 90.8 mm across; a rider carried by velocity spirals off a turntable at **e^(πωh)** a revolution; and a kinematic proxy, *steered* and never teleported, pushes crates at the character's own speed. The controller tunnels through nothing at 10,000 m/s, and Module 8 — thirteen lessons, every piece of physics built rather than imported — is complete. Module 7 is the rotation arc, and it is deliberately the long way round: Euler angles and their twenty-four readings (7.1), the single turn Euler's theorem promises (7.2), the *plane*, where composing a rotation is one multiplication and two mirrors visibly make a turn of twice their angle (7.3), and then quaternions (7.4) — whose multiplication table is **derived** from one sentence about space rather than handed over, and whose half-angle, sandwich and double cover all fall out of a single calculation — and (7.5) **slerp, and the storage swap**: `a·(a⁻¹b)ᵗ` for the fourth time in three lessons, because it is the definition of a geodesic on any group rather than a fact about quaternions, and then the half with no analogue in the plane. The double cover gives every pair of orientations *two* arcs and **49.6%** of random pairs name the wrong one; one dot-product comparison fixes it, and the bug without it is a character spinning **359°** between two keyframes a degree apart. The same fact then pays a dividend nobody advertises: because that comparison confines the arc to one hemisphere, nlerp's schedule error — *unbounded* in the plane, where 7.3 measured **13,131×** — is capped here at **exactly 2**, and at **8.1491°** of pose. The cost and the guarantee are the same fact. Two measurements then refused the claim they were written to confirm: the lesson's own metric had to be **rewritten before anything could be measured**, because 7.4's `2·acos|a·b|` is blind near zero and scored slerp's own geodesic at **−41.10% of its own arc**; and a table of small-arc errors printed a clean `0.0000e+00` for a function returning *NaN*, because `std::max(x, NaN)` returns `x`. `transform::rotation` is now a `quat`, twenty-five call sites moved, and the narrower type found **three places that had never put a rotation in a field called `rotation`** — one shipping in the engine since 5.11 and one a published demo whose camera boom had been storing a *sheared* basis since 5.12, repaired not with a wider field but with one more node. The golden render came back **byte-identical**. Then (7.6) **skeletal animation**, where the module stops being about rotation and starts being about a character. Skinning carries more *vocabulary* than mathematics, so the vocabulary goes and the one matrix that matters is derived from **Lesson 2.8's naming convention alone**: `model_from_joint · joint_from_model`, whose inner labels cancel because they name the same joint at two different *times* and whose outer labels are the same space — so a skinning matrix is `model_from_model`, which is simultaneously the licence to add four of them together and the reason the sum is not a rotation. The inverse bind matrices then cost **no matrix inversion at all**, because inverting a product reverses it and the inverse chain composes with the same flat loop on the other side — finally writing the `local_from_parent` Lesson 2.8 declined to write until a caller existed. At the bind pose every skinning matrix is exactly the identity, so **one number tests a whole rig** with no reference image: `4.768e−07`, against a control that nudges one bind by 0.01 and goes four orders louder. And the artifact is the module's half-angle met a third time — a twisted limb collapses to `cos(θ/2)` of its radius, *exactly zero* at a half-turn, reopening to `cos(θ/2n)` across **n** joints, which is what a forearm twist bone is; a bend obeys the same cosine on the *minor axis of an ellipse*. Two findings refused the expected answer: the one-liner that linear blend skinning is “nlerp without the normalise” is wrong **in both directions**, and a benchmark came back **18.1× too slow** because `cmake -S . -B build` leaves `CMAKE_BUILD_TYPE` empty and a harness's `-O2` does not reach the library it links. The CPU software rasterizer is finished end to end, the same scene is drawn on a real GPU through SDL_GPU, and the tree is now a static library with a public API, a platform/application layer, its own logging, handle-based resource storage, an asset system that can load, share and free, a reusable A/B timing harness that has decided three architecture questions with numbers instead of folklore, and **a working from-scratch ECS** — a generational entity id honoured by every pool, sparse-set component storage, queries that lead with the smallest pool, a **transform hierarchy** whose resolve cost is flat in depth with a camera that is an ordinary entity, (5.10) an **input layer that knows what the player meant** rather than which key they hit, (5.11) **a debug-draw system and a tooling UI** — a queue of world-space geometry with lifetimes that anything in the engine can fill, and Dear ImGui behind a 182-line facade — and (5.12) **a game**, which is Module 5's checkpoint and the first program in this repository written from *outside* the boundary Lesson 5.1 drew by somebody who did not already know where everything was. It keeps a list of what the published headers could not do, and there are **seven**. Two are promises the engine had already made: `engine.hpp`, which calls itself *"the whole public API, in one include"*, listed **40 of the 55 headers it could have** — missing the entire ECS, the asset store, handles, the logger and the action map — and it rotted for a mechanical reason worth more than the bug, namely that **nothing in the repository includes it**, so it was never compiled and nothing could ever fail; it is completed and then *checked*, at configure time, by a glob used as a lint. The measurement justifying that comes out **backwards**: the broken umbrella was *cheaper* (0.28 s) than including exactly what the game uses (0.37 s), because the fourteen headers it omitted are the templated ones — incomplete and cheap are indistinguishable on a stopwatch. The second promise is `scene.hpp`'s, made in 5.1 — *"Module 5's ECS replaces the struct with components"* — kept here by `gfx/renderable.hpp`, and **at two callers rather than three, against this engine's own stated rule**, because what is duplicated is not an idiom but a known-broken `mat4`→`transform` conversion whose predicted removal never happened; the round trip is then proved exact to **zero**, with a control that fires at 1.0. The camera is the lesson's derivation: parent it to the player and you have a follow camera for free, and it is *wrong*, inheriting 22° of cosmetic roll and a non-uniform scale — and the boom that cancels both has one trap, which this file shipped. **The inverse of a product reverses**, so the unscale comes first; written the way the English reads it agrees on *x* to four decimals and is out by **36%** on *y*, and the symptom was not a tilted horizon but a **headless run that hung for ever**, because `SDL_assert` is a retry loop and a build server has no dialog to answer it. The unit cube spans ±0.5 while the icosahedron has radius 1, which cost three separate factor-of-two bugs in one file. And the five findings left open — no collision (the wall is a clamp and the pillars are scenery you drive through, with their bounding boxes drawn from the very matrix a solver would use), no text (the HUD is SDL's debug font and vanishes headless), no audio, no public path from an entity to the GPU renderer, and an ECS `view()` with no `const` overload — are named, measured and dated **at hour 62 instead of hour 434**, which is the whole reason a checkpoint exists — and, opening Module 6, (6.1) **a colour pipeline that is correct at both ends**, which found and fixed a four-module-old bug that had every GPU-rendered pixel too dark, (6.2) **a shading equation with units** — a BRDF measured in inverse steradians, a light measured in irradiance, and a hemisphere integrator that turns "is this model physically plausible?" into a number — (6.3) **a story about what a surface is**: three microfacet distributions and a masking term, each held to an identity that has a right-hand side, and (6.4) **a physically-based BRDF, live in both renderers** — Cook–Torrance with its `4(n·l)(n·v)` denominator *derived* rather than quoted, Fresnel built from the physics with `F0` read off an index of refraction, the metallic workflow arriving as a consequence, and energy conservation measured at a worst hemispherical reflectance of 0.9255 where the model it replaced reached 1.4300, (6.5) **a material system** &mdash; one home for what a surface is, referencing its textures by handle rather than by pointer, with the rule that decides membership set by the hardware rather than by taste &mdash; and (6.6) **a glTF 2.0 loader**, text and binary, with node hierarchies, multi-primitive meshes and the full metallic-roughness material model arriving through the asset store with *no conversion at all*, because three of the four convention checks against this engine come out as "do nothing" &mdash; (6.7) **per-pixel normals in both renderers**, from a tangent frame derived out of the uv chart rather than copied, with a texture that finally knows whether it holds colour or data, and (6.8) **shadows in both renderers** &mdash; a z-buffer rendered from the light, an orthographic projection derived from an interval remap, and a bias that is *derived rather than tuned*: shadow acne turns out to be a sampling error whose magnitude follows from half a texel of lateral travel times the surface's slope, predicted at 50% of a lit plane and measured at 50.1%, with the worst real error reaching 92% of the derived bound, and (6.9) **cascaded shadow maps** — the light's box fitted to a slice of the *camera's* frustum rather than to the scene, with the two artefacts that only exist once that box moves: a corner-fitted box changes size by **30.2%** when the camera merely turns (cured by a bounding sphere, measured spread 9.9&times;10⁻⁸, at a cost of 29% of the resolution), and a sliding texel grid makes every edge crawl (cured by snapping, drift 0.499 texels → 7.6&times;10⁻⁶). Its real subject is an **audit**: 6.8 derived its bias from `world_per_texel`, cascades change that quantity by 7.3&times;, and not one line of the derivation had to move — while the *device-space* bias turns out to be constant across cascades because the sphere's radius cancels, with the one exception the mechanism predicts. It ends by measuring a scene where cascades **lose**, and (6.10) **mipmapping, trilinear filtering and anisotropy** — a debt Lesson 3.9 measured and deferred, and that four later lessons repeated, once in a *shipped public header*. The level is `log2` of the footprint, and the footprint is the CPU-specific part: a GPU shades in 2×2 quads so `ddx` is a subtraction, but a scanline rasterizer has no neighbour, so the derivative comes from the triangle by the quotient rule — **exact**, and checked against a central difference of the real interpolation to 5.79×10⁻⁶. It also quantifies the famous linear-light bug: a chain averaged in sRGB bytes delivers **42.2%** of the light it should at level 1 and compounds, which is why the symptom is a surface that dims as it recedes, and (6.11) **transparency in both renderers** &mdash; which is not a new feature but a *correctness gap in shipped code*: the pipeline had said `no blending` since 4.4 and the glTF importer ignored `alphaMode` entirely, so every downloaded asset with foliage or glass rendered as opaque cardboard, silently, and alone among that importer's gaps it fired no status &mdash; because a status says *the file wants what the engine cannot do*, and there was no blend state to be the other operand. Alpha is **coverage**, not opacity, and everything follows: `over` is an area-weighted average, therefore a lerp, therefore linear-light, so half-coverage white over black is code **188** where lerping stored bytes gives **128** and delivers **42.9%** of the light &mdash; and the *hardware* makes the same mistake, measured at 188 against 127 between an `_SRGB` target and a `UNORM` one, discharging a prediction left in a shader comment six lessons earlier. Masking costs **zero** pipelines and one reordering (the depth write moves after the fragment &mdash; the same dependency that costs a GPU its early-Z); blending costs **six**, because a second axis of state multiplies rather than adds. Premultiplied alpha turns out to be for *filtering* rather than for compositing, a coverage-preserving mip chain stops foliage dissolving with distance (**31.2%** of the leaves gone by level 6 without one), and the engine gains its first per-frame sort &mdash; with the ceiling of per-object ordering named rather than glossed, and (6.12) **an HDR pipeline in both renderers** &mdash; which opens by establishing that the engine had no HDR *bug*: it had been **avoiding the question by construction**, because 6.2 set `k_reference_irradiance` to &pi; so a white surface renders at exactly 1.0 and the demo's roughness of 0.49 peaks at 0.8676, just under. Change that one number and the same shading equation returns **55,917** for a polished metal &mdash; 15.8 stops &mdash; every bit of which an 8-bit target stores as code 255. Exposure arrives on the photographic EV scale, which locates the old behaviour at EV &minus;0.263 rather than beside the new scale as a special case; Reinhard is derived in one line and its white point from requiring f(W)=1, while ACES is named honestly as a five-constant *fit* whose toe costs 13 codes at the dark end. The resolve is a **second render pass** with no vertex buffer &mdash; one triangle rather than two, and 4.1's quad traversal says why &mdash; because an exposure derived from the frame cannot be known while the frame is being drawn. And it pays off 6.11: compositing on a float target is **8.1&times;** cheaper, because that decode-and-re-encode round trip was never a cost of blending, it was a cost of storing something other than light, and (6.13) **a bloom in both renderers and a post-processing stack** &mdash; which opens by measuring that 6.12 stopped bright values being destroyed by the *fragment shader* but did not make them **visible**. Bisected through the engine's own encode, the lid only *moved*: ACES saturates at **6.3774**, so that polished metal still spends **13.10 stops** on a single code &mdash; and no curve can do better, because an 8-bit sRGB display holds **11.69 stops in total**. Note which operator has the *highest* lid: Reinhard, at 223.5, and it is the one everybody agrees looks worst, because it spends its codes creeping toward a white it never reaches. So tonemapping is not the removal of clipping &mdash; it is the *choice of where to clip*, and the way past it is a **change of variable**. A real lens scatters, so a point brighter than white lands as a spike with wide skirts, and the skirts fall *below* the lid even when the source is far above it: the display cannot put 10,000 in a pixel's intensity, so it puts it in how many pixels are lit. That is a prediction with an exponent rather than a metaphor &mdash; if the tail falls as r&#8315;&sup2; then the glow's **area** is linear in luminance &mdash; and over three decades it holds to within **8.6%**. The pyramid then turns out to be not merely a cheap approximation to a wide Gaussian but a *better shape* than one, because a sum of Gaussians whose widths double has a power-law envelope: measured slope **&minus;2.01**, and over one octave the pyramid falls **6.0&times;** where a single Gaussian falls **403&times;**. Both filters are derived rather than chosen &mdash; one bilinear tap placed at the shared corner *is* the 2&times;2 box average, and a box convolved with a box *is* the 1-2-1 tent &mdash; and the soft knee follows from three continuity conditions, with the flicker it cures living in the *derivative* rather than the value. It ends on the two questions `gpu_post.hpp` deferred in its own shipped words: **who owns the intermediates** (a pass that knows its own lifetimes owns them; the stack owns only what crosses between stages) and **what makes that answer stop scaling** &mdash; stated plainly, with eleven render passes for one effect as the number Module 6's frame graph lesson now has to justify itself against, and (6.14) **antialiasing, both kinds** &mdash; which begins by separating two problems that share a name. Geometric aliasing undersamples <em>coverage</em>, a step function whose spectrum has energy at every frequency, so <em>no sample rate resolves it</em>: supersampling a checker above Nyquist drives the per-pixel error from 0.5000 to 0.0231 and never recovers the checker, because it is not representable on the grid at any factor. Shading aliasing undersamples the <em>lighting</em>, and that one can be located exactly, because the GGX lobe has a closed form &mdash; solve the NDF for its half-maximum and <code>sin t = &alpha;&radic;((&radic;2&minus;1)/(1&minus;&alpha;&sup2;))</code>, three lines, exact at every roughness and agreeing with a bisection of the real NDF to <strong>1.6&times;10&#8315;&#8312;</strong>, with a coefficient of 0.6436 that shows the familiar &ldquo;about &alpha; wide&rdquo; to be <strong>55%</strong> too big. Set against how far a normal turns across one pixel, the crossover is a number: <strong>roughness 0.2468</strong> on a 40-pixel sphere, which puts the demo&rsquo;s 0.49 safely above and every polished metal from 6.12 below. MSAA cannot reach that half <em>by construction</em> &mdash; it multisamples coverage and shades once per primitive per pixel, the asymmetry that makes it affordable &mdash; and the measured gap against true supersampling reaches a factor of 300. The cure is to prefilter the distribution, and the measurement reframes the whole subject: it improves per-pixel <em>accuracy</em> by 2.4&times; and sometimes makes it worse, while improving <em>stability</em> by <strong>2,034&times;</strong>. <strong>Antialiasing does not make a pixel correct; it makes it stable</strong> &mdash; because the artefact was never a wrong value but a value that moves, and (6.15) **image-based lighting and a skybox in both renderers** &mdash; which retires the constant `ambient` the engine has carried since 3.6. Two things were wrong with it and only one was ever written down: it has <em>no direction</em>, so a face turned up and one turned down receive identical fill; and it has <em>no specular half at all</em>, because `base * ambient` passes through no BRDF and a metal has no `base` &mdash; a sentence shipped in `light.hpp` and `scene.frag.hlsl` since 6.4, naming this lesson by number as the fix. An environment map replaces it with radiance as a <em>function of direction</em>, and the same texture is the sky you see and the light you receive, so a reflection <em>cannot</em> disagree with its surroundings. The cube's geometry hides a correctness bug: a face is flat, so d&omega;&nbsp;=&nbsp;dA/r&sup3; and a corner texel is <strong>1/(3&radic;3)</strong> of a centre one &mdash; averaging with equal weights is 1.01% too bright, and <em>sixteen times the texels does not move it</em>, which is what makes it a bias rather than an error. Then two integrals, and only one is exact: Lambert's BRDF does not depend on the incoming direction, so it leaves the integral entirely, converges second-order to <strong>1.1&times;10&#8315;&#8309;</strong>, and reproduces 6.2's `albedo&nbsp;&times;&nbsp;ambient` end to end &mdash; the old term is a special case, not a casualty. The specular half does <em>not</em> factor and the split sum pretends it does, at a price measured against brute force and <em>decomposed</em> into what the bake lost and what the technique loses: 0.08% at roughness 0.10, under 5.8% head-on, <strong>29.0% dark</strong> at roughness 1 seen edge-on, and <strong>73.0%</strong> once the sky has a sun in it &mdash; an asymmetry that <em>is</em> the `n = v = r` assumption, and a conclusion not in the paper: the error depends on the environment, not only on the material. Meanwhile the whole BRDF half collapses to one 64&sup2; table serving every scene ever, because Schlick's Fresnel is linear in F0, and the NDF vanishes from the integrator entirely, cancelled against its own sampling density. And the roughness&rarr;mip mapping everybody writes with no argument attached turns out to be <em>derivable</em> from 6.14's lobe width &mdash; and to be right to within <strong>0.77 of a level</strong>, and (6.16) **frustum culling and instanced submission** &mdash; the engine finally acting on the counters `draw_stats` has kept honestly since 4.8. The GPU <em>does</em> reject what is off screen, but it rejects it in the <strong>clipper</strong>, downstream of the vertex shader, so a prop two kilometres behind the camera costs a full vertex dispatch, a pipeline bind, two uniform pushes and a draw call first. Six dot products stop that &mdash; and the interesting part is where the planes come from: the <em>rows</em> of the view-projection matrix, by the single observation that a clip coordinate is already a signed distance, so <code>x &ge; &minus;w</code> rearranges to <code>(row0 + row3) &middot; p &ge; 0</code>, which <em>is</em> a plane. No trigonometry, and &mdash; the actual reason &mdash; no second copy of the truth to drift from the matrix the renderer uses. The near plane is the one that is a convention: SDL_GPU clips <code>0 &le; z &le; w</code>, so it is <strong>row 2 alone</strong> where every OpenGL-derived article writes <code>row2 + row3</code>. The extraction is verified against facts needing no reference implementation &mdash; the eye is the <em>apex</em>, so its distance to all four side planes is exactly <strong>0.000000</strong> &mdash; and immediately against the fact that <em>four zeros are not enough</em>, because a reversed normal passes all four, which is the bug the first draft made. An honest digression chases the far plane being 1.4&times;10&#8315;&sup3; out of place: the obvious culprit is cancellation in <code>row3 &minus; row2</code>, and redoing it in <code>double</code> reproduces the <code>float</code> answer to seven digits, which <em>rules it out</em> &mdash; the error is upstream in <code>perspective()</code>, where <code>A + 1</code> with <code>A = &minus;1.003009</code> amplifies one ulp by <strong>332&times;</strong>. Then both directions in which a bounding test is conservative, including the corner case where a box passes all six half-space tests with a different corner each time: <strong>0.69% of kept boxes</strong> against <strong>zero</strong> false rejects, and that asymmetry is the entire licence for the technique. The measurement that matters is <em>not</em> the cull rate, which measures the scene: a sweep over object count reported &ldquo;cull wins&rdquo; six times and meant nothing, because both sides are O(n). The crossover is in the <strong>cull rate</strong>, predicted as <code>c/w</code> = <strong>0.2446%</strong> &mdash; one object in 409 &mdash; and then bracketed. The survivors become an instance buffer, and the second half is as honest: a batch key is <em>five</em> things, so this engine's own demo scene makes <strong>12 batches from 12 objects</strong>; the 112-byte matrix block does not shrink, it <em>changes road</em>, and the whole 720-byte saving is fifteen materials not pushed. <strong>Instancing saves submission, not bandwidth.</strong> Sixteen draw calls become one, the fragment shader is reused byte for byte, and the frame comes back <strong>bit-identical across 76,800 channels</strong> &mdash; an instrument that had to be <em>built</em>, because the golden is blind to this lesson and the page says so instead of taking the unchanged hash as a pass, and (6.17) **a lightweight frame graph** &mdash; because the engine can now record <strong>seventeen render passes</strong> in a frame and there is no single place where a frame happens: three types begin their own passes, two are handed one, and the rules keeping them in order live in <em>prose</em>, in three headers, checked by nothing. The move it rests on is refusing to let a name mean a texture &mdash; &ldquo;the bloom reads the HDR target&rdquo; does not identify a picture, since the image before the skybox and the image after it are both it &mdash; so a resource becomes a <em>variable with values over time</em>, each version with exactly one producer, which is <strong>single static assignment</strong> applied to render targets. From one statement per pass, four facts are then <em>derived</em> rather than maintained: the ordering edge, the load op, the <em>producer's</em> store op, and the lifetime. The load op is the case that stings, because 6.13 wrote a paragraph warning what happens when you get one wrong and described the symptom as looking &ldquo;like a tuning problem rather than a load op&rdquo;; here it is not a decision. Every derived op matches what the engine chose by hand across eight attachments &mdash; including 4.7's <code>DONT_CARE</code> on the scene depth and 6.8's <code>STORE</code> on the shadow map, argued out nine lessons apart and now both consequences of one rule. Culling is backward reachability from imported writes, so <strong>eleven bloom passes disappear</strong> because nothing reads them, with no flag anywhere. Then the honest part, which reframes the subject: the usual pitch for a frame graph is <em>memory aliasing</em>, and measured on this frame it saves <strong>zero</strong> &mdash; for two reasons that must be told apart, since SDL_GPU 3.4.12 has no placed resource or heap, <em>and</em> our frame is a <em>chain</em> in which almost nothing is dead while anything else is alive. Perfect aliasing would reach <strong>85.7%</strong>, an unreachable <strong>5.27&nbsp;MB</strong> at 1920&times;1080 which turns out to be exactly the bloom pyramid. A compile costs <strong>8.62&nbsp;&micro;s</strong> and <strong>zero heap allocations</strong>, measured by replacing the global <code>operator new</code>, which is the argument against <code>std::function</code> made with a number. Five silent mistakes become named compile errors, and the compiled frame is <strong>bit-identical across 262,144 channels</strong> &mdash; with a control reporting 196,608 differing on a bloom-less frame, because one lesson ago a golden was found that printed <code>identical=YES</code> when both its file reads failed, and (6.18) **text and a 2D overlay, on both renderers** &mdash; closing Module 6 with the observation that this engine can evaluate a microfacet BRDF, cascade a shadow map, prefilter an environment cube, bloom, tonemap, cull and deduce its own frame order, and cannot put a single character on the screen. Every number the module measured went to a terminal, so <strong>the picture and the measurement have never once been on the same surface</strong>. Fixing that is three problems and only one is about drawing. The first is <em>arithmetic</em>: pen, baseline, bearing, advance &mdash; and what to round, where the same rounding applied to two quantities has completely different consequences. Snapping a glyph's POSITION discards the fraction for display only, so the pen keeps it and the error is bounded at <strong>0.4765 px</strong> forever; rounding its ADVANCE discards it from the accumulator, so every error is added to the next &mdash; a random walk still <strong>4.32 px</strong> out at the end of one 45-character line. The second is <em>packing</em>: ninety-five glyphs on six shelves, 6.10's padding argument arriving at <em>magnification</em> (without it, <strong>47.3%</strong> of edge texels carry a neighbour's ink, one bilinear tap away), and an occupancy figure of 43.6% that looks like a verdict on the packer and is really a fact about the font, because its denominator is a power of two fixed before the packer ran &mdash; the honest number, over the rows it touched, is <strong>83.3%</strong>. The third is what the course has been building toward since 1.6: compositing a glyph is <code>over</code>, <code>over</code> is a lerp, and a lerp between light values must happen in linear light. Text is the worst case, because a 16 px glyph is mostly edge and the error is largest where coverage is <em>lowest</em> &mdash; a tenth-covered pixel emits <strong>10.3%</strong> of the light it should, so the tapers and the flanks of every stem very nearly vanish while the fully-covered interiors come out right. Over a whole string, light text on dark carries <strong>62.2%</strong> of the correct ink and dark text on light <strong>137.8%</strong>: the same error in opposite directions, which is why half the people who see it conclude the font is too spindly and half that it is too heavy, and neither suspects the compositor. Then the finding that fell out of putting two numbers side by side &mdash; an <code>_SRGB</code> coverage atlas, a <em>texture format</em> bug in a different file, produces <strong>62.1%</strong>, the same number to four significant figures, because both are <code>srgb_to_linear</code> applied to the coverage at different points in the pipeline. Two independent bugs, numerically indistinguishable on a dark background, separated by one test the lesson derives: invert the contrast, because a blend-space bug is directional and a coverage bug is not. The GPU half is measured rather than argued: the same draw at a plain UNORM target with a shader-side encode reproduces the deliberately-wrong CPU function on <strong>0 differing pixels</strong>, discharging a prediction 6.1 left in a comment. Where the overlay composites is settled with a number &mdash; white at linear 1.0 through the ACES fit is sRGB code <strong>232</strong>, not 255, and it moves with the exposure, so a HUD composited before the curve dims when the player walks into a dark room. The overlay is then declared as a frame-graph pass in <em>one word</em>, the first use of 6.17's API from outside &mdash; including the <code>keep</code> on an imported resource that had been written and never run &mdash; with the honest report that declaring the same pass <code>discard_write</code> compiles happily and erases the frame under the text: <strong>a graph relocates a dataflow claim, it does not verify it</strong>. 98 checks; the same batch composited by the software rasterizer and by SDL_GPU agreeing on <strong>every pixel to within one sRGB code</strong>, with a control reporting 934 differing so the agreement means something; a steady-state frame of text that allocates <strong>nothing</strong>; and <code>hello_cube</code> &mdash; two hundred lines, no GPU device &mdash; finally able to say what it is doing. Then **7.7 — sampling and blending**, where the pose finally comes from *data*: a clip is a function from time to pose, and because it does not <em>store</em> poses (3.6 MB for thirty seconds of one character) every decision in the lesson falls out of the substitution. The per-channel keyframe layout arrives with the number the argument does not predict — <strong>a naive bake is 30% BIGGER than a pose array</strong>, because every key carries its own time — and the point that the layout is not the saving but what <em>makes</em> the saving possible: <strong>53 of 69 channels</strong> on a 23-joint walk carry nothing, for a final 7.7&times;. Sampling is a bracket search that playback's coherence lets you skip, 2.74&times; to 10.34&times; faster than a binary one; the three interpolation rules are decided one per <em>field</em>, including the scale question 7.5 left open, settled by the arithmetic-minus-geometric mean gap, by <code>ln(0) = &minus;&infin;</code> and by glTF's <code>LINEAR</code> mode; and looping turns out to have two traps, a duration that must be <em>authored</em> (one frame short is <strong>5.82&deg;</strong> of tick per cycle) and a <code>std::fmod</code> that takes the sign of the dividend and is <em>not constant time</em>. The finding the lesson is built around: <strong>a cross-fade is not 7.6's weighted sum</strong>. Blend the poses and every bone keeps its length to <strong>2.980e&minus;07</strong>; blend the composed matrices — identical inputs, one step later — and the limb shortens by <code>&Sigma; cos((k&minus;1)&delta;/2)</code>, matched to <strong>4.768e&minus;07</strong>, with the <em>accumulated</em> angle in place of the per-joint one so the hips are untouched and the hands and feet are ruined. Two real renders, <strong>5.00000 against 4.65020</strong>. And (7.8) <strong>audio</strong> closes the module with the first subsystem whose output you <em>cannot look at</em> &mdash; which changes what the work is, because a bug you cannot see has to be measured, on a thread you do not own, against a deadline that does not negotiate. The arithmetic is the simplest in the course and everything difficult follows from one line of it: <strong>mixing is addition</strong>. Sum sixteen uncorrelated voices and the level grows as <strong>&radic;N, not N</strong> &mdash; measured at exactly 0.577&middot;&radic;N, with a control mixing the <em>same</em> voice N times that lands on N exactly &mdash; so the obvious <code>1/N</code> headroom spends <strong>12 dB</strong> it did not have to; but the <em>peak</em> obeys neither law and keeps growing the longer you listen (6.98&times; at ten milliseconds, <strong>10.46&times; at eight seconds</strong>), which makes headroom a bet rather than a bound. Pan with the first law anybody writes and a sound crossing the image ducks <strong>3.01 dB</strong> in the middle, because two speakers add in <em>power</em>; attenuate with <code>1/d&sup2;</code> &mdash; the right law for the wrong quantity &mdash; and everything falls off at <strong>&minus;12 dB per doubling instead of &minus;6</strong>, because a sample is a pressure and pressure is the square root of intensity. Then 7.7's reconstruction rule arriving where nobody expected it: <strong>the gain is lossy too</strong>, sampled at 60 Hz and reconstructed at 48 kHz, and a zero-order hold injects <strong>0.1007 in one sample</strong> &mdash; 7.7&times; the largest step the waveform itself ever takes &mdash; of which only <strong>1.2%</strong> sits at the tone's own frequency, so what you hear is sixty broadband impulses a second. Sixty-four voices cost <strong>29.76 &micro;s of a 10,667 &micro;s deadline</strong> and one log line costs 1.02 &micro;s and is unbounded, which is why the audio thread counts instead of logging &mdash; and a dropout counter that reported <strong>28 of 58 buffers starved</strong> on a perfectly healthy run turned out to be measuring SDL3's pull model working correctly. Nine measured sections, twelve controls, and <strong>no audio device open</strong> for any of them. Start at
...
./build/demos/collector                   # Lesson 5.12's game: drive, collect twelve orbs
...
# Lesson 7.4 — the two things an algebra shows that a matrix cannot
./build/demos/gimbal --commute 90 --shot out.ppm   # both orders: 120 deg apart
./build/demos/gimbal --cover 360 --shot out.ppm    # home, with q at -1
...
`gimbal` is **Lesson 7.1's** instrument and has become **Module 7's**, which is why it is still the smallest program here rather than the fourth demo in a row: one aircraft, three hoops, and a live readout of `|det J|` — the determinant of the map from knob rates to angular velocity, which is `cos(pitch)` and which goes to zero at gimbal lock. Hold <kbd>E</kbd> to drive yaw and roll together (the weakest combination), climb toward pitch 90°, and watch the knobs keep turning while the aircraft stops. Press <kbd>T</kbd> first and the nose leaves a trail, so the stopping is a mark on the screen rather than something you have to notice. **7.2** added <kbd>A</kbd> (pose the craft by the *single* turn Euler's theorem promises, and print how far that disagrees with the three-knob route — it never exceeds a few millionths of a degree) and <kbd>B</kbd> (two ghost craft travelling from the same start to the same end, one lerping three angles and one turning steadily about one axis; **watch what stays still**). **7.4** added the two things an *algebra* can show that a matrix cannot. <kbd>C</kbd> performs the same two turns in both orders and draws both journeys, which form a quadrilateral that **fails to close** — at 90° each the two orders end **120.0000° apart**, printed three ways that share no code. <kbd>D</kbd> turns the craft through two full revolutions beside a dial that goes round **once**: at 360° the readout says the craft is `0.000 deg` from where it started and the quaternion is `(-1, 0, 0, 0)`, which is the double cover with a number attached.
...
version, as of **Lesson 8.1**:

```
engine/include/engine/   the public API — 90 headers, and the only path a demo can name
...
engine/include/engine/phys/       integrate — three rules over a position and a
                                  velocity, and the diagnostics that price them (8.1)
...
demos/hello_cube/        the public-API acceptance test — and, since 6.18, the
                         smallest program in the tree that can say what it is doing
...
demos/integrate/         Lesson 8.1: three integrators in PHASE SPACE, because a
                         bouncing dot cannot tell them apart for the first few seconds
